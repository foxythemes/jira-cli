...
export default class JiraIssues {
  ...
  */
  createIssue(newIssue) {
    ...
    jira.api.addNewIssue(newIssue)
      .then((issue) => {
        const config = jira.config.defaults;
  ...
  */
  createIssueObj(options = {}) {
    ...
    this.getMetaData().then((meta) => {
      ...
      for (const index in meta.projects) {
      ...
      inquirer.prompt(project).then((answers1) => {
        const projectIssueTypes = [];

        // Get issue types of the selected project
        for (const i in issueTypes[selectedProject]) {
        ...
        const questions = [
          {
            ...
            filter(val) {
              return projectIssueTypes.find((obj) => obj.name == val);
            },
          },
          {
            type: 'input',
        ...
        inquirer.prompt(questions).then((answers2) => {
          ...
          if (answers2.issueType.subtask) {
            ...
            inquirer.prompt(question).then((answers3) => {
            ...
            _this.getFields().then((data) => {
              const epicNameField = data.filter((val) => val.custom && val.name == 'Epic Name');
  ...
  */
  search(args) {
    const _this = this;
    jira.api.searchJira(`summary ~ '${args}'`).then((r) => {
      if (r.total) {
  ...
  */
  summary(user) {
    ...
    jira.api.searchJira(jql).then((r) => {
  ...
  */
  getReleaseIssues(options) {
    const _this = this;
    jira.api.searchJira(`project = ${options.project} and fixVersion = ${options.release}`).then((r) => {
      if (r.total) {
        _this.showIssues(r.issues);
      }
    }).catch((res) => {
  ...
  */
  findIssue(issue) {
    ...
    jira.api.findIssue(issue).then((r) => {
      _this.showIssue(r);
    }).catch((res) => {
      jira.showErrors(res);
      process.exit();
  ...
  */
  getProjectIssues(project) {
    const _this = this;
    jira.api.searchJira(`project = ${project} and resolution = Unresolved`).then((r) => {
      if (r.total) {
        _this.showIssues(r.issues);
      }
    }).catch((res) => {
  ...
  async getIssueTransitions(issueId) {
    return jira.api.listTransitions(issueId)
      .then((res) => {
        ...
        for (const index in transitionObj) {
          transitions.push({
  ...
  */
  async makeTransition(issueId, transitionName) {
    ...
    if (typeof transitionName === 'string' && transitionName.length > 0) {
      const transition = transitions.find((obj) => obj.name == transitionName);
      ...
      const question = [
        {
          ...
          choices: transitions,
          filter(val) {
            return transitions.find((obj) => obj.name == val);
          },
        },
      ];
  ...
  */
  addComment(issueId, comment) {
    return jira.api.addComment(issueId, comment).then((res) => {
